...
// ========== ДОПОЛНИТЕЛЬНЫЕ ПОЛЕЗНЫЕ ФУНКЦИИ ИЗ app.js ==========
...
function clearAppCache() {
    ...
    if (window.GameData) {
        ...
        console.log('🔄 Сброшено состояние GameData');
    }

    console.log('✅ Кэш очищен, перезапускаем приложение...');
...
async function authorize() {
    try {
        ...
        GameData.token = data.data.token;
 ...
 * @param {string} type - Тип обратной связи ('correct', 'incorrect', 'tap')
 */
...
document.addEventListener('DOMContentLoaded', () => {
    // Используем расширенную инициализацию если доступна
    if (typeof initAppWithAuth === 'function') {
        initAppWithAuth();
    } else {
        initApp();
    }
